...
import numpy
from RectangleSearcher import RectangleSearcher
...
class Unet:
    ...
    # bta - binarisation trehold accurate
    #-------------------------------------------------------------------------------
    def store_segmented_images(self,
        ...
        for y in range(0, self.height):
            for x in range(0, self.width):
                if array_probabilities_binary[y, x] > bta:
    ...
    #---------------------------------------------------------------
    # segments an input image using fast heat map with a more
    # intelligent approach by searching for regions of interest
    # args:
    ...
    #           segmentation window
    # btc - binarisation threshold coarse 
    # bta - binarisation threshold accurate
    ...
    def segment_fast(self,
        ...
        # segments with offset 0, 0 to cover basics
        self.segment_with_offset(batch_size, input_image.height, input_image.width, 0, 0, padding,
                                 input_image_extended_array, segmented_array_probabilities_sum,
                                 segmented_array_probabilities_count, segmented_partially_name_prefix)

        # segments with offset windows_size, window_size to cover basics
        self.segment_with_offset(batch_size, input_image.height, input_image.width,
                                 self.window_size, self.window_size, padding,
                                 input_image_extended_array, segmented_array_probabilities_sum,
                                 segmented_array_probabilities_count, segmented_partially_name_prefix)

        # binarisation (stage 1 - getting coarse binarisation results)
        segmented_array_probabilities_binary = segmented_array_probabilities_sum / \
            segmented_array_probabilities_count

        for y in range(0, self.height):
            for x in range(0, self.width):
                if segmented_array_probabilities_binary[y, x] > btc:
                    segmented_array_probabilities_binary[y, x] = 255.0
                else:
                    segmented_array_probabilities_binary[y, x] = 0

        # now do regions retrieval
        rectangle_searcher = RectangleSearcher()
        list_of_regions = rectangle_searcher.search_for_roi(segmented_array_probabilities_binary, self.window_size)        

        # use again segment_with_offset
        segmented_array_probabilities_sum = numpy.zeros((input_image.height, input_image.width))
        segmented_array_probabilities_count = numpy.zeros((input_image.height, input_image.width))
        segmented_array_probabilities_binary = numpy.zeros((input_image.height, input_image.width))

        # do loop across regions of interest
        for roi_tuple in list_of_regions:

            # setup boundaries for windowed based segmentation
            xmin = roi_tuple[0] - self.window_size
            ymin = roi_tuple[1] - self.window_size
            xmax = roi_tuple[2] + self.window_size
            ymax = roi_tuple[3] + self.window_size

            if xmin < 0:
                xmin = 0
            if ymin < 0:
                ymin = 0
            if xmax > input_image.width:
                xmax = input_image.width
            if ymax > input_image.height:
                ymax = input_image.height

            for offset_relative_x in range(0, 2 * self.window_size, step):
                for offset_relative_y in range(0, 2 * self.window_size, step):

                    # we are only segmenting a part of the image
                    # between xmin, ymin and xmax, ymax
                    # with windows and step declared earlier
                    offset_x = xmin + offset_relative_x
                    offset_y = ymin + offset_relative_y
